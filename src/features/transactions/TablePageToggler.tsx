    ...
    <>
      ...
      <div className="text-right -mt-2 py-3 select-none hidden lg:block">
        ...
        <ChevronLeftIcon
          ...
          width={24}
          height={24}
          className={`inline-block -mt-1 ${canPreviousPage ? 'text-base-content cursor-pointer' : 'text-gray-500'}`}
        />
        <ChevronRightIcon
          onClick={() => gotoPage(pageIndex + 1)}
          width={24}
          height={24}
          className={`inline-block -mt-1 ${canNextPage ? 'text-base-content cursor-pointer' : 'text-gray-500'}`}
        />
      </div>
      ...
      <div className="select-none mb-16 lg:hidden">
        <div className="rounded-t overflow-hidden border border-dark-700 bg-dark-800 flex justify-between p-3">
          ...
          <span className="text-base-content"> {summaryString}</span>{' '}
        </div>
        <div className="border-b rounded-b border-l border-r border-dark-700 bg-dark-900 overflow-hidden flex justify-between">
          <div
            ...
            className={`flex-grow border-r p-3 border-dark-700 ${
              canPreviousPage ? 'text-base-content cursor-pointer' : 'text-gray-500'
            }`}
          >
            {i18n._(t`Previous`)}
          </div>
          <div
            onClick={() => gotoPage(pageIndex + 1)}
            className={`flex-grow p-3 text-right ${canNextPage ? 'text-base-content cursor-pointer' : 'text-gray-500'}`}
          >
            {i18n._(t`Next`)}
          </div>
        </div>
