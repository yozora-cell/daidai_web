...
const Item = ({
        ...
        >
          <div className="indicator">
            <div className="avatar">
              <div className="w-8 rounded-full bg-base-100">
                    ...
                    {imgurl ? (
                      ...
                      <>
                        <Davatar
                          size={32}
                          address={`${address}`}
                          defaultComponent={<Identicon seed={`${address}`} className="!w-8 !h-8 rounded-full" />}
                          provider={library}
                        />
                      </>
                    )}
  ...
  return (
    <div className="box-border w-full p-3 transition border rounded-md bg-base-100 border-base-300 hover:shadow-lg">
      <div className="box-border flex flex-row items-center justify-between w-full">
        <div className="-space-x-6 avatar-group !overflow-visible	">
          {data && data.owner && data.owner.account != AddressZero ? (
            <>{getAvatarLink(data.owner.account ?? '', data.owner.avatar_url, ownerTip, `/account`, 0)}</>
          ) : (
            <></>
          )}
          {getAvatarLink(
          ...
          <Link href={`/collection/${data?.chainId}/${data?.contract}/${data?.tokenId}`}>
            <a className="w-full">
              <div
                ...
                data-tip={`${data?.name}`}
              >
                <Typography variant="base" className="w-10/12 truncate" weight={700}>
                  {data?.name}
                </Typography>
              </div>
            </a>
      ...
      <div className="w-full mt-2">
        {listingItem && targetToken ? (
          <Typography variant="sm" className="inline-flex text-base-content" weight={700}>
        ...
        {/* <Typography variant="sm" className="inline-flex ml-4 text-base-content text-opacity-60">
          1/1
        </Typography> */}
      </div>
