...
import { Trade } from 'app/types/daidai'
import Link from 'next/link'

const Item = ({ data, index }: { data: Trade; index: number }) => {
  const { account, chainId } = useActiveWeb3React()
  return (
    <>
      <tr className="hover:shadow">
        <th>
          <Link href={`/collection/${chainId}/${data.address}`}>
            <a className="w-full">
              <div className="flex flex-row items-center justify-start gap-2">
                <Typography weight={700}>{index}</Typography>
                <div className="avatar">
                  <div className="w-12 rounded-full">
                    <Image src={data.avatar ?? defaultImg} alt="avatar" layout="responsive" width={64} height={64} />
                  </div>
                </div>
              </div>
            </a>
