...
import NFT_MARKETPLACE_ABI from 'app/constants/abis/daidai-nft-marketplace.json'
...
import ERC721_ABI from 'app/constants/abis/erc721.json'
import FACTORY_ABI from 'app/constants/abis/factory.json'
import GACHA_ABI from 'app/constants/abis/gacha.json'
import INARI_ABI from 'app/constants/abis/inari.json'
import INO_ABI from 'app/constants/abis/ino.json'
...
import MULTI_TRANSFER_ABI from 'app/constants/abis/multi-transfer.json'
...
export function useInoContract(address: string, inoChainid: number, withSignerIfPossible = true): Contract | null {
  const { chainId } = useActiveWeb3React()
  let contractAddress = chainId && inoChainid && chainId == inoChainid ? address : undefined
  return useContract(contractAddress, INO_ABI, withSignerIfPossible)
}

export function useNFTMarketplaceContract(withSignerIfPossible = true): Contract | null {
  const { chainId } = useActiveWeb3React()
  return useContract(chainId ? NFT_MARKETPLACE_ADDRESS[chainId] : undefined, NFT_MARKETPLACE_ABI, withSignerIfPossible)
}

export function useNFTContract(nftAddress: string, withSignerIfPossible = true): Contract | null {
  return useContract(nftAddress, ERC721_ABI, withSignerIfPossible)
}

export function useMultiTransferContract(withSignerIfPossible = true): Contract | null {
  const { chainId } = useActiveWeb3React()
  return useContract(chainId ? MULTI_TRANSFER_ADDRESS[chainId] : undefined, MULTI_TRANSFER_ABI, withSignerIfPossible)
}

export function useGachaContract(withSignerIfPossible = true): Contract | null {
  const { chainId } = useActiveWeb3React()
  return useContract(chainId ? GACHA_ADDRESS[chainId] : undefined, GACHA_ABI, withSignerIfPossible)
}
