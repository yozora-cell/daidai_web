...
const ListPanelFooter: FC<ListPanelFooterProps> = ({ title, value }) => {
  return (
    <div className="flex flex-row items-center justify-between px-4 py-2">
      <Typography variant="xs" weight={400} className="text-base-content">
        {title}
      </Typography>
      {value && (
        <Typography className="text-base-content" variant="sm" weight={700}>
          {value}
        </Typography>
      )}
    </div>
...
const ListPanelHeader: FC<ListPanelHeaderProps> = ({ title, value, subValue, className = '', id }) => {
  return (
    <div id={id} className={classNames(className, 'flex flex-row justify-between px-4 py-[10px] items-center')}>
      <Typography variant="lg" className="text-base-content" weight={700}>
        {title}
      </Typography>
      {(value || subValue) && (
        <div className="flex flex-col text-right">
          {value && (
            <Typography className="text-base-content" weight={700}>
              {value}
            </Typography>
          )}
          {subValue && (
...
const ListPanelItemLeft: FC<ListPanelItemLeftProps> = ({ amount, hideCurrencyLogo, startAdornment }) => {
  return (
    <div className="flex flex-row gap-1.5 lg:gap-3 items-center">
      ...
      <Typography variant="sm" className="text-base-content" weight={700}>
...
const CurrencyAmountItem: FC<CurrencyAmountItemProps> = ({
  ...
  return (
    ...
    >
      ...
      <Typography className="text-right text-base-content" weight={700}>
