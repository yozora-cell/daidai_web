      ...
      return (
        <div className="avatar">
          <div className="w-12 rounded-full">
            <Image width={48} height={48} layout="responsive" src={data.avatar_url} alt="account avatar" />
          </div>
        </div>
      )
    }
    return (
      <>
        <Davatar
          size={48}
          address={account ?? ''}
          defaultComponent={<Identicon seed={account ? account : ''} className="!w-12 !h-12 rounded-full" />}
          provider={library}
        />
      </>
    )
      ...
      <div className="space-y-3">
        ...
        <HeadlessUiModal.BorderedContent className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            {connectorName}
            <Button variant="outlined" color="primary" size="xs" onClick={deactivate}>
              {i18n._(t`Disconnect`)}
            </Button>
          </div>
          <div id="web3-account-identifier-row" className="flex flex-col justify-center gap-4">
            <div className="flex items-center gap-4">
              <div className="overflow-hidden rounded-full">{getAvatar()}</div>
              <Typography weight={700} variant="lg" className="text-base-content">
        ...
        <HeadlessUiModal.BorderedContent className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            ...
            <Button variant="outlined" color="primary" size="xs" onClick={clearAllTransactionsCallback}>
              {i18n._(t`Clear all`)}
            </Button>
          </div>
