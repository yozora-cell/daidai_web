...
import { checkedTransaction, finalizeTransaction, removeTransaction } from './actions'
        ...
        promise
              ...
              addPopup(
                {
                  txn: {
                    ...
                    summary: receipt.status === 1 ? transactions[hash]?.summary : `There was an error in the transaction "${transactions[hash]?.summary}".`,
                  },
                },
                hash
              )
          ...
          .catch((error) => {
            if (!error.isCancelledError) {
              console.error(`Failed to check transaction hash: ${hash}`, JSON.stringify(error))
              dispatch(
                removeTransaction({
                  chainId: chainId,
                  hash: hash,
