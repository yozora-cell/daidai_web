...
export interface ApplicationState {
  ...
  readonly isActionsOpen: { [key: string]: boolean }
}
...
export default createReducer(initialState, (builder) =>
  builder
    .addCase(updateBlockNumber, (state, action) => {
      // console.log('updateBlockNumber', action)
    ...
    .addCase(setIsActionsOpen, (state, action) => {
      const { key, isActionsOpen } = action.payload
      for (let curKey in state.isActionsOpen) {
        if (curKey !== key) {
          state.isActionsOpen[curKey] = false
        }
      }
      state.isActionsOpen[key] = isActionsOpen
    })
)
