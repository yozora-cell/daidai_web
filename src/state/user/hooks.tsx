...
/**
 * select authToken from state
 * @returns
 */
export function useAuthToken(account: string | null | undefined): string | null {
  const authToken = useSelector<AppState, AppState['user']['authToken']>((state) => state.user.authToken)
  return account && authToken ? authToken[account.toLowerCase()] : null
}
...
export function useSetAuthToken() {
  ...
  const setAuthToken = useCallback(
    (account: string, authToken: string | null) => {
      // console.log('setAuthToken', account, authToken)
      dispatch(updateAuthToken({ account: account, authToken: authToken }))
    },
    [dispatch]
  )
