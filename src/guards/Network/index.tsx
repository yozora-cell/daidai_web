...
const Component: FC<NetworkGuardProps> = ({ children, feature, asModal = true }) => {
  ...
  const link = (
    <NavLink href="/">
      <a className="text-info focus:outline-none">{i18n._(t`home page`)}</a>
    </NavLink>
  )
  ...
  const content = (
    <div className="flex justify-center lg:mt-[200px]">
      <div className="flex flex-col justify-center gap-5 p-4 mt-10 lg:mt-0">
        <Typography variant="h1" className="max-w-2xl text-center text-primary" weight={700}>
          {/*@ts-ignore TYPE NEEDS FIXING*/}
          {i18n._(t`This feature is not yet supported on the this network`)}
        </Typography>
        <Typography className="text-center max-w-[248px] mx-auto">
          {/* <Trans id="Either return to the {link}" values={{ link }} components={Fragment} />{' '} */}
          {i18n._(t`change to an available network`)}
        </Typography>
        ...
        <div className="grid grid-cols-[repeat(2,_100px)] md:grid-cols-[repeat(4,_100px)] gap-y-10 justify-center">
          {supportedNetworks.map((key: string, idx: number) => (
            <button
              className="flex flex-col items-center justify-center gap-2 text-primary hover:text-info"
              key={idx}
              onClick={() => {
                // @ts-ignore TYPE NEEDS FIXING
                const params = SUPPORTED_NETWORKS[key]
                cookie.set('chain-id', key)
            ...
            >
              ...
              <Typography variant="sm" weight={700} className="text-primary">
                {/*@ts-ignore TYPE NEEDS FIXING*/}
                {NETWORK_LABEL[key]}
              </Typography>
            </button>
