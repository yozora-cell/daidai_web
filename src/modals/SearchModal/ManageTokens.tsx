      ...
      userAddedTokens.map((token) => (
        <div className="flex justify-between px-4 py-3 hover:bg-dark-800/40" key={token.address}>
          ...
          <div className="flex items-center gap-2">
            <div
              className="flex items-center w-4 h-4 cursor-pointer hover:text-primary"
              onClick={() => removeToken(chainId, token.address)}
            >
              <Trash width={20} />
            </div>
            <ExternalLinkIcon
              className="flex items-center cursor-pointer min-w-4 min-h-4 hover:text-primary"
    ...
    <>
      <input
        ...
        className={classNames(
          searchQuery !== '' && !isAddressSearch ? 'border-accent' : 'border-base-300 hover:border-base-content',
          'w-full border rounded placeholder-secondary font-bold text-base p-4 appearance-none'
        )}
        value={searchQuery}
        autoComplete="off"
      ...
      <HeadlessUiModal.BorderedContent className="flex flex-col !p-0">
        <div className="flex items-center justify-between p-4">
          <Typography variant="xs" weight={700} className="text-secondary">
            {userAddedTokens?.length} Custom {userAddedTokens.length === 1 ? 'Token' : 'Tokens'}
          </Typography>
          <Button variant="outlined" color="primary" size="xs" onClick={handleRemoveAll}>
            {i18n._(t`Clear all`)}
          </Button>
        </div>
