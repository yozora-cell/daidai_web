      ...
      <SwapLayoutCard>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="z-0 flex justify-center -mt-6 -mb-6">
            <div
              role="button"
              className="p-1.5 rounded-full bg-dark-800 border shadow-md border-dark-700 hover:border-dark-600 bg-base-100"
              onClick={() => {
                setApprovalSubmitted(false) // reset 2 step UI for approvals
                onSwitchTokens()
              }}
            >
              <ArrowDownIcon width={14} className="text-info hover:text-primary" />
            </div>
          </div>
          <SwapAssetPanel
            spendFromWallet={true}
          ...
          {swapIsUnsupported ? (
            <Button color="error" disabled fullWidth className="rounded-2xl md:rounded">
              {i18n._(t`Unsupported Asset`)}
            </Button>
          ) : !account ? (
            <Web3Connect color="primary" variant="filled" fullWidth className="rounded-2xl md:rounded" />
          ) : showWrap ? (
            <Button
              fullWidth
              color="primary"
            ...
            <div>
              ...
              {approvalState === ApprovalState.APPROVED && (
                <Button
                  color={isValid && priceImpactSeverity > 2 ? 'error' : 'default'}
                  onClick={() => {
                    if (isExpertMode) {
                      handleSwap()
                    } else {
            ...
            <Button
              color={isValid && priceImpactSeverity > 2 && !swapCallbackError ? 'error' : 'default'}
              fullWidth
              onClick={() => {
                if (isExpertMode) {
                  handleSwap()
