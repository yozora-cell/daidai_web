...
import UserItem from 'app/features/ranking/UserItem'
import NetworkGuard from 'app/guards/Network'
import { useProfileTrades } from 'app/services/apis/hooks'
...
const Ranking = () => {
  ...
  const { data, error } = useProfileTrades(page, limit, daytype, orderField, orderDirection, selectedChainId)
      ...
      <div className="container">
        <div className="flex flex-col mt-12 md:flex-row md:items-center">
          <div className="flex flex-col w-full gap-4 px-6 sm:flex-row sm:justify-start sm:items-center">
            <h1 className="text-2xl font-bold sm:text-3xl text-base-content">{i18n._(t`User Ranking`)}</h1>
            <DayFilter
                  ...
                  <>
                    <table className="table w-full">
                      <thead className="border-b">
                        <tr>
                          <th className="w-20 bg-base-100">{i18n._(t`User`)}</th>
                        ...
                        {list.map((item, index) => {
                          return (
                            <>
                              <UserItem
                                data={item}
                                key={`${index}/${item.account}/trade/${item.updatedAt}`}
                                index={index + 1}
                              ></UserItem>
                            </>
                          )
